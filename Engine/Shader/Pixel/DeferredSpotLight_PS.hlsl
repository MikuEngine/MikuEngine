...
// Spot Light: sample GBuffer, L vector, distance/angle attenuation, PBR BRDF
float4 main(PS_INPUT input) : SV_Target
{
    ...
    float3 l = lVec / max(dist, EPSILON);
    float nDotL = saturate(dot(n, l));
    // no discard when nDotL <= 0: SSS bleeds past terminator for soft boundary
