...
namespace game
{
    ...
    public:
        ...
        void PlayFadeOut(float durationOverride = -1.0f);
    ...
    private:
        ...
        void CacheTargets();
        void SetAlphaRecursive(engine::Transform* t, float a);
    ...
    private:
        ...
        float m_time = 0.0f;
        float m_duration = 0.18f;

        bool m_animating = false;

        // --- Toast Enter settings ---
        float m_enterSlideOffsetX = 600.0f; // 오른쪽 밖에서 시작할 X 오프셋
        float m_enterDuration = 0.18f;

        // --- Reflow settings ---
        float m_moveDuration = 0.12f;

        // --- Fade settings ---
        float m_fadeDuration = 0.20f;
