...
namespace game
{
    ...
    class BossBigProjectile :
        public engine::Script<BossBigProjectile>,
        public IDamageable
    {
        REGISTER_SCRIPT(BossBigProjectile, Script)
    ...
    private:
        ...
        // 체력 시스템
        // ─────────────────────────────────────────────
        float m_Hp = 1.0f;  // 현재 체력 (Setup에서 BossScript로부터 설정)
        
        // ─────────────────────────────────────────────
        // 반사 설정 (Setup에서 BossScript로부터 설정)
        // ─────────────────────────────────────────────
        float m_reflectDamage = 200.0f;                // 반사 데미지
        float m_reflectLifetimeExtension = 10.0f;      // 반사 시 수명 연장
    ...
    public:
        ...
        // ─────────────────────────────────────────────
        // 초기화 (BossScript에서 모든 설정 가져오기)
        // ─────────────────────────────────────────────
        void Setup(const engine::Vector3& direction, BossScript* boss);

        // ─────────────────────────────────────────────
        // 결정화/처형 시스템
        // ─────────────────────────────────────────────
        void OnCrystallized();  // 결정화 처리 (false→true 전환 시 호출)
