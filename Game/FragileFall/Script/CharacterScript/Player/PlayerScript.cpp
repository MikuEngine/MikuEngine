...
namespace game
{
	...
	// 컴포넌트 캐싱
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::CacheComponents()
	{
	...
	void PlayerScript::CheckForwardBack(engine::Vector3& forward, engine::Vector3& aimDir)
	{
	...
	// 입력 처리 (입력 → FSM 파라미터)
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::ProcessInput()
	{
	...
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::UpdateGameLogic()
	{
	...
	// Rigidbody 이동, 회전 등
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::UpdatePhysicsLogic()
	{
	...
	// 상태 변화 콜백
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::OnStateEntered(const std::string& state)
	{
		...
		if (state == "Execution")
		{
			// 처형 상태 진입 시 이동 정지 (Kinematic: 자체 속도 초기화)
			...
			if (m_isDashing)
			{
				...
				m_dashVelocity = engine::Vector3::Zero;
				m_dashCollisionDecayBoost = 1.0f;
			}
		}
	}

	void PlayerScript::StartExecution(engine::GameObject* targetMonster)
	{
		...
		if (monsterTransform)
		{
			...
			{
				m_rigidbody->ForceSetPosition(targetPos, false);
			}
	...
	// FSM 초기화
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::InitializeFSM()
	{
	...
	void PlayerScript::InitializeAnimFSM()
	{
		if (!m_animFSM) return;
	...
	void PlayerScript::InitializeAnimations()
	{
	...
	// 입력 유틸리티
	// ═══════════════════════════════════════════════════════════════
	engine::Vector3 PlayerScript::GetMoveInputDirection() const
	{
	...
	// ═══════════════════════════════════════════════════════════════
	bool PlayerScript::CheckEnvironmentBlock(const engine::Vector3& direction, engine::Vector3& outNormal)
	{
		// 유효하지 않은 방향이면 리턴
	...
	engine::Vector3 PlayerScript::CalculateSlidingDirection(
	...
	float PlayerScript::CalculateDashSpeed() const
	{
		// ═══════════════════════════════════════════════════════════════
	...
	void PlayerScript::HandleDash()
	{
		...
		if (CheckEnvironmentBlock(m_dashDirection, wallNormal))
		{
			...
			m_dashVelocity = m_dashDirection * reducedSpeed;
		}
		else
		{
			// 현재 대쉬 속도 계산 (지수 감쇠)
			float dashSpeed = CalculateDashSpeed();
			m_dashVelocity = m_dashDirection * dashSpeed;
		}

		// ═══════════════════════════════════════════════════════════════
		// Kinematic: Transform 직접 이동
		// ═══════════════════════════════════════════════════════════════
		engine::Transform* transform = GetTransform();
		if (!transform) return;

		engine::Vector3 displacement = m_dashVelocity * fixedDelta;
		...
		if (m_rigidbody && !m_rigidbody->IsDynamic())
		{
			m_rigidbody->ForceSetPosition(newPos, false);
		}
		else
		{
	...
	void PlayerScript::HandleShooting(float deltaTime)
	{
		// ─────────────────────────────────────────────
		// 쿨다운 타이머 감소 (항상 실행)
		// ─────────────────────────────────────────────
	...
	std::string PlayerScript::GetAnimationState() const
	{
	...
	// 에디터 검증
	// ═══════════════════════════════════════════════════════════════
	bool PlayerScript::ValidateComponents() const
	{
		bool isValid = true;
	...
	// GUI / 직렬화
	// ═══════════════════════════════════════════════════════════════
	void PlayerScript::OnGui()
	{
		ImGui::Indent();

		ImGui::Text("PlayerScript (Kinematic):");
		...
		if (rigidbody)
		{
			...
			{
				ImGui::TextColored(ImVec4(1, 0.5f, 0, 1), "[WARNING] Rigidbody is Dynamic! This script requires Kinematic.");
			}
			else
			{
				ImGui::TextColored(ImVec4(0, 1, 0, 1), "[OK] Rigidbody: Kinematic");
			}
		}
		else
		{
	...
	void PlayerScript::Save(engine::json& j) const
	{
	...
	void PlayerScript::Load(const engine::json& j)
	{
