...
namespace game
{
    ...
    void MonsterRoundBlue::Start()
    {
        ...
        {
            meshRenderer->SetBaseColor(engine::Vector4(0.0f, 0.0f, 1.0f, 1.0f));
        }
    }
    ...
    {
        // Blue: 기본 Linear 총알 (추후 특수 패턴으로 변경 가능)
    ...
    // 공격 (Blue 전용)
    // ═══════════════════════════════════════════════════════════════
    void MonsterRoundBlue::Attack(float deltaTime)
    {
        // 공격 애니메이션 타이머 업데이트
    ...
    void MonsterRoundBlue::OnGui()
    {
        ...
        ImGui::TextColored(ImVec4(0.0f, 0.0f, 1.0f, 1.0f), "Tier: Blue");
